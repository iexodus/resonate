...
<div data-body-class="home flat_background has_user_menu topnav_root" class="projects panel home_tab project_index<?php if(empty($cards) && empty($list) && empty($updates)) echo ' blank_slate';?><?php if(!$create_projects && empty($cards) || empty($list) || empty($updates)) print ' no_nav';?>">
...
<!--                   <div id="next_slide" class="next" style="display: none"></div>
        ...
        <div class="">
        <h3><span class="ss-icon ss-tag"></span>Categories</h3>
        <ul class="navigation">
            <?php foreach ($categories as $category){?>
            <li><a class="link_page" href="<?=$base_url?>/projects/<?=$category->name?>?category=<?=$category->tid?>"><?=$category->name?></a></li>
            <?php }?>
        </ul>
        
    </div>
        <div class="list_tags">
            <h3><span class="ss-icon ss-tag"></span>Tags</h3>
        <ul class="navigation">
            <?php foreach ($tags as $tag){?>
            <li><a class="link_page" href="<?=$base_url?>/projects/<?=$tag->name?>?tag=<?=$tag->tid?>"><?=$tag->name?></a></li>
            <?php }?>
        </ul>
            ...
            <div class="form_search">
                <form action="<?=$base_url?>/projects" method="post">
                    <input type="text" name="key_word" placeholder="Search Text" value="<?=isset($_POST['key_word'])?$_POST['key_word']:''?>"><input type="submit" name="search_description" value="Search">
                </form> 
        </div>
